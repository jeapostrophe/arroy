...
(define-unit ghost@
  ...
  (struct challenged (letters challenger challenged) #:transparent)
  (struct defending (letters challenger challenged) #:transparent)
  (struct end (loser) #:transparent)
  ...
  (define add-letters (for/list ([c (in-list (string->list "ABCDEFGHIJKLMNOPQRSTUVWXYZ"))])
                        (add c)))
    ...
    (match-lambda*
      [(list (middle letters p _) p)
       (let* ([moves add-letters]
              [moves (if (> (length letters) 0) (cons (challenge) moves) moves)]
              [moves (if (> (length letters) 2) (cons (declare) moves) moves)])
         moves)]
  ...
  (define render
    (match-lambda*
      ...
      [(list (challenged letters er ee) er)
       (format "You challenged player ~a on ~a." ee (letters->string letters))]
      [(list (challenged letters er ee) ee)
       (format "You were challenged by player ~a on ~a." er (letters->string letters))]
      [(list (challenged letters er ee) _)
       (format "Player ~a challenged player ~a on ~a." er ee (letters->string letters))]
      [(list (defending letters er ee) ee)
       (letters->string letters)]
      [(list (defending letters _ ee) _)
       (format "Player ~a is attempting to finish ~a." ee (letters->string letters))]
      [(list (end p) p)
       "You were eliminated."]
      [(list (end p) _)
...
(module+ main
  (define-values/invoke-unit/infer (export arroy^) (link ghost@ arroy@))
  (play 2))